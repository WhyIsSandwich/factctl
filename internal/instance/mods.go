...
func (mm *ModManager) InstallModsRecursively(ctx context.Context, inst *Instance, modNames []string) ([]string, error) {
	...
	for len(installQueue) > 0 {
		...
		if err := mm.InstallMod(ctx, inst, modName); err != nil {
...
func (mm *ModManager) getModDependencies(ctx context.Context, inst *Instance, modName string) ([]string, error) {
	...
	for _, dep := range modInfo.Dependencies {
		// Skip base mod and optional dependencies
		if dep == "base" || strings.HasPrefix(dep, "?") {
			continue
		}
...
func (mm *ModManager) installModFromPortal(ctx context.Context, inst *Instance, modName string) error {
	...
	if err := mm.downloadFromPortal(ctx, modName, &buf); err != nil {
	...
	fmt.Printf("  → Successfully installed '%s' from portal\n", modName)
	return nil
}
...
// installModFromRegistry searches all sources for a mod and installs it
func (mm *ModManager) installModFromRegistry(ctx context.Context, inst *Instance, modName string) error {
	fmt.Printf("  → Searching for mod '%s' in registries...\n", modName)
	
	// Try each source as a registry
	for sourceName, sourceURL := range inst.Config.Mods.Sources {
		fmt.Printf("  → Checking registry '%s' (%s)...\n", sourceName, sourceURL)
		
		// Download the source and search for the mod
		var repoBuf bytes.Buffer
		if err := mm.downloadMod(ctx, sourceURL, &repoBuf); err != nil {
			fmt.Printf("  → Warning: Failed to download registry '%s': %v\n", sourceName, err)
			continue
		}
		
		// Search for the mod in this registry
		modData, err := mm.findModInRepository(repoBuf.Bytes(), modName)
		if err != nil {
			// Mod not found in this registry, try next one
			continue
		}
		
		fmt.Printf("  → Found mod '%s' in registry '%s'\n", modName, sourceName)
	...
	return fmt.Errorf("mod '%s' not found in any registry", modName)
}
...
func (mm *ModManager) downloadFromPortal(ctx context.Context, modName string, buf *bytes.Buffer) error {
	...
	if modInfo.LatestRelease == nil {
		return fmt.Errorf("no releases found for mod: %s", modName)
	}
	
	fmt.Printf("  → Found mod '%s' version %s on portal\n", modName, modInfo.LatestRelease.Version)
	
	// Download the mod file
	downloadURL := fmt.Sprintf("https://mods.factorio.com%s", modInfo.LatestRelease.DownloadURL)
	
	// Note: This will redirect to login page without authentication
	// In a real implementation, we'd need to get username/token from player-data.json
	// or implement Web Authentication API
